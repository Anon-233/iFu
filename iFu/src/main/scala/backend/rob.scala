        ...
        for (i <- 0 until numWritePorts) {
            ...
            when((io.wb_resps(i).valid && MatchBank(GetBankIdx(rob_idx)) &&
                    !robVal(GetRowIdx(rob_idx)))){
                        printf(p"------------------------assert failed here------------------------\n")
                        printf(p"pc = ${io.wb_resps(i).bits.uop.debug_pc}\n")
                        printf(p"rob_idx = ${rob_idx}\n")
                        printf(p"robVal = ${robVal(GetRowIdx(rob_idx))}\n")
                        printf(p"MatchBank = ${MatchBank(GetBankIdx(rob_idx))}\n")
                        printf(p"------------------------end---------------------------------------\n")
                    }

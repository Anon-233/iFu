...
abstract trait DecodeTable {
    ...
    //     |     |  |         |        |        |       |       |       |  |     |  |  |  |  |  |      |    |  |  |  |  |  csr cmd
    //     |     |  |         |        |        |       |       |       |  |     |  |  |  |  |  |      |    |  |  |  |  |  |
        List(N, uopX, IQT_X, FU_X, RT_X, DC1, DC1, immX, X, X, X, X, N, /*M_X,*/ DC2, X, X, N, N, X, CSR.N) //TODO CSR.N改为CSR.X
...
//    io <> DontCare
   ...
   /* when (!uop.is_sfb_shadow && cs.uopc === uopORI && inst(21,10) === 0.U) {
    ...
    when(cs.csr_cmd =/= CSR.N){
        printf(p"Find a CSR instruction, pc = ${io.enq.uop.debug_pc}\n")
    }
    assert(cs.csr_cmd === CSR.N)
}

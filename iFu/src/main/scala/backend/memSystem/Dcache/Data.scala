...
class DataReq extends CoreBundle with HasDcacheParameters {
    ...
    val writePos = UInt(log2Ceil(nWays).W) // 将要写入的路
}

class DataResp extends CoreBundle with HasDcacheParameters {
    val rdata = Vec(nRowWords, UInt(xLen.W))
}
...
class DcacheData extends Module with HasDcacheParameters{
    ...
    // data
    val data = Array.fill(nSets)(SyncReadMem(nWays, Vec(nRowWords, UInt(xLen.W))))
    ...
    when (reseting) {
        val resetData = VecInit(Seq.fill(nRowWords)(0.U(xLen.W)))
        for (i <- 0 until nSets) {
            when (resetIdx === i.U) {
                for (j <- 0 until nWays) {
    ...
    for (w <- 0 until memWidth) {
        ...
        readValid(w) := io.lsuRead(w).req.valid || io.mshrRead.req.valid ||
                        io.replayRead.req.valid || io.fenceRead.req.valid
        ...
        readPos(w) := Mux(io.lsuRead(w).req.valid, io.lsuRead(w).req.bits.readPos,
                      ...
                      Mux(io.fenceRead.req.valid,  io.fenceRead.req.bits.readPos,
                                                   0.U))))
    }

    for (w <- 0 until memWidth) {
        val rdataChannel = Wire(Vec(nSets, Vec(nRowWords, UInt(xLen.W))))
    ...
    val writeData  = Wire(Vec(memWidth, Vec(nRowWords, UInt(xLen.W))))

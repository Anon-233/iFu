...
class MSHR extends CoreModule {
    val io = IO(new Bundle {
        val req_pri_val = Input(Bool())     // 首个请求有效
...
class IOMSHR extends CoreModule {
    val io = IO(new Bundle {
    ...
    io.cbusReq.axiBurstType := AXI_BURST_FIXED
    io.cbusReq.size         := MSIZE4
    io.cbusReq.addr         := 0xbad.U
    io.cbusReq.data         := 0xbad.U

    val s_idle :: s_mem_access :: s_resp :: Nil = Enum(3)
    val state = RegInit(s_idle)

    io.req.ready := state === s_idle

    val req = Reg(new DCacheReq)
    val read_req = isRead(req.uop.mem_cmd)

    val mmio_data = Reg(UInt(xLen.W))
    ...
    } .elsewhen (state === s_mem_access) {
        io.cbusReq.valid   := true.B
        io.cbusReq.isStore := !read_req
        io.cbusReq.mask    := Mux(read_req, 0.U, req.wmask)
        io.cbusReq.addr    := req.addr
        io.cbusReq.data    := Mux(read_req, 0.U, req.data)

        when (io.cbusResp.ready && io.cbusResp.isLast) {
            mmio_data := io.cbusResp.data
            state     := Mux(read_req, s_resp, s_idle)
        }
    } .elsewhen (state === s_resp) {
        io.resp.valid     := true.B
        io.resp.bits.uop  := req.uop
        io.resp.bits.data := mmio_data
        when (io.resp.fire) {
            state := s_idle
        }
    }

...
class NonBlockingDCache extends CoreModules {
    ...
    val t_replay :: t_wb :: t_lsu :: t_refill :: Nil = Enum(4)

    val mshrs   = Module(new MSHRFile)
    val wbu     = Module(new WritebackUnit)
    val rfu     = Module(new RefillUnit)
    val cbusArb = Module(new CBusArb)

    mshrs.io.brupdate := io.lsu.brupdate
    // mshrs.io.exception    := io.lsu.exception
    mshrs.io.wb_req   <> wbu.io.req
    mshrs.io.wb_resp  <> wbu.io.resp
    mshrs.io.ref_req  <> rfu.io.req
    mshrs.io.ref_resp <> rfu.io.resp
    cbusArb.io.mmio_req  := mshrs.io.cbusReq
    mshrs.io.cbusResp    := cbusArb.io.mmio_resp
    cbusArb.io.wb_req    := wbu.io.cbusReq
    wbu.io.cbusResp      := cbusArb.io.wb_resp
    cbusArb.io.ref_req   := rfu.io.cbusReq
    rfu.io.cbusResp      := cbusArb.io.ref_resp
    io.cbusReq           := cbusArb.io.cbus_req
    cbusArb.io.cbus_resp := io.cbusResp

    // meta
    val meta = Module(new MetaArray)
    val metaWriteArb = Module(new Arbiter(new MeatWriteReq, 1))
    // 0 -> MSHR refills
    val metaReadArb = Module(new Arbiter(new DualMetaReadReq, 3))
    // 0 -> wb, 1 -> MSHR replays, 2 -> pipeline
...
// -------------------------------------------------------------------------------
    ...
    io.lsu.req.ready := metaReadArb.io.in(2).ready && dataReadArb.io.in(2).ready
    metaReadArb.io.in(2).valid := io.lsu.req.valid
    dataReadArb.io.in(2).valid := io.lsu.req.valid
    for (w <- 0 until memWidth) {
        // Tag read for new requests
        metaReadArb.io.in(2).bits.req(w).idx    := io.lsu.req.bits(w).bits.addr >> lineOffBits
        metaReadArb.io.in(2).bits.req(w).wayMux := DontCare
        metaReadArb.io.in(2).bits.req(w).tag    := DontCare // 读出来再判断是不是hit 或许是VIPT
    ...
    // 这里还是一个请求占两个端口
    metaReadArb.io.in(2).valid              := mshrs.io.replay.valid
    metaReadArb.io.in(2).bits.req(0).idx    := mshrs.io.replay.bits.addr >> lineOffBits
    metaReadArb.io.in(2).bits.req(0).wayMux := DontCare
    metaReadArb.io.in(2).bits.req(0).tag    := DontCare

    dataReadArb.io.in(2).valid              := mshrs.io.replay.valid
    dataReadArb.io.in(2).bits.req(0).addr   := mshrs.io.replay.bits.addr
    dataReadArb.io.in(2).bits.req(0).wayMux := mshrs.io.replay.bits.wayMux
    dataReadArb.io.in(2).bits.valid         := widthMap(w => (w == 0).B)    // 只读第一个口

    // -----------
    // Write-backs
    val wb_fire = wbu.io.meta_read.fire && wbu.io.data_req.fire
    ...
    metaReadArb.io.in(0).valid        := wb.io.meta_read.valid
    metaReadArb.io.in(0).bits.req(0)  := wb.io.meta_read.bits

    dataReadArb.io.in(0).valid        := wb.io.data_req.valid
    dataReadArb.io.in(0).bits.req(0)  := wb.io.data_req.bits
    dataReadArb.io.in(0).bits.valid   := widthMap(w => (w == 0).B)

    wb.io.meta_read.ready := metaReadArb.io.in(0).ready && dataReadArb.io.in(0).ready
    wb.io.data_req.ready  := metaReadArb.io.in(0).ready && dataReadArb.io.in(0).ready
    assert(!(wb.io.meta_read.fire ^ wb.io.data_req.fire))

    // -----------
    // Refill
    val refill_fire = rfu.io.meta_write.fire && rfu.io.data_write.fire
    val refill_req = Wire(Vec(memWidth, new DCacheReq))
    refill_req             := DontCare
    refill_req(0).uop      := NullMicroOp
    refill_req(0).addr     := Cat(rfu.io.meta_write.bits.tag, rfu.io.data_write.bits.addr)
    refill_req(0).data     := DontCare

    metaReadArb.io.in(1).valid        := widthMap(w => true.B)
    metaWriteArb.io.in(1).valid       := rfu.io.meta_write.valid
    metaWriteArb.io.in(1).bits.req(0) := rfu.io.meta_write.bits

    dataReadArb.io.in(1).valid        := widthMap(w => true.B)
    dataWriteArb.io.in(1).valid       := rfu.io.data_write.valid
    dataWriteArb.io.in(1).bits.req(0) := rfu.io.data_write.bits

    rfu.io.meta_write.ready := metaReadArb.io.in(1).ready && metaWriteArb.io.in(1).ready && dataWriteArb.io.in(1).ready
    rfu.io.data_write.ready := metaWriteArb.io.in(1).ready && dataWriteArb.io.in(1).ready
// -------------------------------------------------------------------------------

    val s0_valid = MuxCase(VecInit(0.U(memWidth.W).asBools), Seq(
        wb_fire || mshrs.io.replay.fire -> VecInit(1.U(memWidth.W).asBools),
        io.lsu.req.fire                 -> VecInit(io.lsu.req.bits.map(_.valid))
    ))

    val s0_req = MuxCase(0.U.asTypeOf(Vec(memWidth, new DCacheReq)), Seq(
        wb_fire              -> wb_req,
        mshrs.io.replay.fire -> replay_req,
        io.lsu.req.fire      -> VecInit(io.lsu.req.bits.map(_.bits))
    ))

    val s0_type  = MuxCase(t_replay, Seq(
        wb_fire              -> t_wb,
        mshrs.io.replay.fire -> t_replay,
        io.lsu.req.fire      -> t_lsu
    ))
    ...
    // 好像都存了所选的way，除了普通的lsu请求和预取请求
    val s1_replay_wayMux         = RegNext(mshrs.io.replay.bits.wayMux)
    val s1_wb_wayMux             = RegNext(wb.io.data_req.bits.wayMux)

    // tag check    // 就是来比较tag的
    val s1_tag_eq_way = widthMap(i => wayMap(
            (w: Int) => (meta(i).io.resp(w).valid &&
                 meta(i).io.resp(w).tag === (s1_addr(i) >> untagBits))
    ).asUInt)
    ...
    val s1_rf_idx_matches = widthMap(
        i => (s1_addr(i)(untagBits - 1,lineOffBits) === rfu.io.idx.bits) && rfu.io.idx.valid
    )

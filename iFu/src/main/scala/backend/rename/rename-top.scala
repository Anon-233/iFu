...
) extends  AbsRenameStage(plWidth,numPhysRegs,numWbPorts)
{
    ...
    for(w <- 0 until plWidth){
        ren2AllocReqs(w) := ren2Uops(w).ldst_val && ren2Uops(w).dst_rtype === rtype && ren2Fire(w)
        ren2BrTags(w).valid := ren2Fire(w) && ren2Uops(w).allocate_brtag

        comValids(w) := io.com_uops(w).ldst_val && io.com_uops(w).dst_rtype === rtype && io.com_valids(w)
        rbkValids(w) := io.com_uops(w).ldst_val && io.com_uops(w).dst_rtype === rtype && io.rbk_valids(w)
